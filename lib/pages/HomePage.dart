...
class _HomePageState extends State<HomePage> {
  ...
  final RegionAnalysisService _regionService = RegionAnalysisService(); // Instanciar
  final ScrapeService _scrapeService = ScrapeService(); // Instanciar com a chave da SerpApi
  ...
  void _exibirRespostaProcessada(Map<String, dynamic> respostaProcessada) {
    ...
    if (respostaProcessada['conteudo_texto'] != null && 
        ...
        (respostaProcessada['conteudo_texto'] as String) != "[CARD DO IMÓVEL]") {
      _appServices.salvarMensagem('assistant', respostaProcessada['conteudo_texto']!);
    }
    ...
    setState(() {
      ...
      if (respostaProcessada['imoveis'] != null &&
        ...
        for (var imovelMap in (respostaProcessada['imoveis'] as List<dynamic>)) {
          ...
          _messages.add({
            "text": "", // O texto visual do card é construído pelo _buildMessage
            "isUser": false,
            "isCard": true,
  ...
  Future<void> _processarAcaoDoCliente(Map<String, dynamic> criterios) async {
    ...
    setState(() {
      ...
      _messages.add({"text": _loadingMessage, "isUser": false, "isCard": false});
    });
    _scrollToBottom();

    try {
      // 1. Chamar RegionAnalysisService
      List<RegiaoSugerida> regioesSugeridas = await _regionService.analisarRegioesComScoreIA(
      ...
      if (regioesSugeridas.isEmpty) {
        setState(() {
          _messages.add({"text": "Não consegui encontrar regiões adequadas com os critérios atuais. Que tal tentarmos outros?", "isUser": false, "isCard": false});
          _isLoading = false;
        });
        _scrollToBottom();
        return;
      ...
      setState(() {
        _loadingMessage = "Regiões promissoras encontradas: $regioesTexto. Buscando imóveis...";
         final ultimaMsgIndex = _messages.length -1; // Atualiza a última mensagem de loading
         if (_messages[ultimaMsgIndex]["text"].toString().contains("Analisando as melhores regiões")) {
            _messages[ultimaMsgIndex] = {"text": _loadingMessage, "isUser": false, "isCard": false};
         } else {
            _messages.add({"text": _loadingMessage, "isUser": false, "isCard": false});
         }
      });
      ...
      int numLinksPorRegiao = 3; // Quantos links da SerpApi processar por região
      int maxImoveisPorRegiao = 2; // Quantos imóveis tentar extrair com sucesso por região
      ...
      for (RegiaoSugerida regiao in regioesSugeridas) {
        ...
        String nomeCompletoRegiao = "${regiao.nomeRegiao}, ${regiao.cidadeRegiao}";
        
        // Construir query para SerpApi
        String querySerpApi = _scrapeService.construirQuerySerpApi(
          templateQuery: criterios['template_query_serpapi'] as String? ?? "{REGIAO_PLACEHOLDER} {TIPO_IMOVEL_PLACEHOLDER} {OBJETIVO_PLACEHOLDER}",
          nomeRegiaoSugerida: nomeCompletoRegiao,
          criteriosIA1: criterios,
        );
        
        List<String> linksImoveis = await _scrapeService.getGoogleLinks(querySerpApi, numResults: numLinksPorRegiao);
        
        int imoveisColetadosNestaRegiao = 0;
        for (String linkImovel in linksImoveis) {
          if (todosOsImoveisColetados.length >= totalImoveisDesejados || imoveisColetadosNestaRegiao >= maxImoveisPorRegiao) break;
          
          Map<String, dynamic>? dadosImovel = await _scrapeService.scrapeEDetalhaImovel(linkImovel);
      ...
      if (todosOsImoveisColetados.isEmpty) {
         setState(() {
            _loadingMessage = "Encontrei algumas regiões interessantes, mas não consegui listar imóveis específicos no momento. Poderia tentar refinar sua busca?";
            final ultimaMsgIndex = _messages.length -1;
            if (_messages[ultimaMsgIndex]["text"].toString().contains("Buscando imóveis")) {
                _messages[ultimaMsgIndex] = {"text": _loadingMessage, "isUser": false, "isCard": false};
            } else {
                _messages.add({"text": _loadingMessage, "isUser": false, "isCard": false});
            }
      ...
      if(mounted) {
        setState(() {
          if (_messages.isNotEmpty && _messages.last["text"].toString().contains("Formatando as melhores opções")) {
      ...
      _exibirRespostaProcessada(respostaFormatadaFinal);
        ...
        setState(() {
          if (_messages.isNotEmpty && _messages.last["text"].toString().contains("Analisando") || _messages.last["text"].toString().contains("Buscando") || _messages.last["text"].toString().contains("Formatando")) {
            _messages.removeLast();
          }
        });
        _exibirRespostaProcessada(_userServices.formatoErroPadrao("Ocorreu um problema durante a busca detalhada de imóveis. Tente novamente ou com outros critérios."));
    } finally {
  ...
  void _sendMessage() async {
    ...
    if (respostaN8NInteracao1['action_tag'] == 'ANALYZE_REGIONS_AND_SCRAPE_LISTINGS' &&
        respostaN8NInteracao1['status'] == 'action_required_client' &&
        respostaN8NInteracao1['payload_criteria'] is Map) { // <--- ESTA CONDIÇÃO AGORA DEVE SER VERDADEIRA!
        ...
        _processarAcaoDoCliente(respostaN8NInteracao1['payload_criteria'] as Map<String, dynamic>);
      ...
      print("HomePage: Formato de resposta N8N Interação 1 inesperado: $respostaN8NInteracao1");
      _exibirRespostaProcessada(_userServices.formatoErroPadrao("Recebi uma resposta inesperada do assistente."));
      setState(() => _isLoading = false); // Termina o loading
    }
  }
  ...
  Widget _buildMessage(Map<String, dynamic> message) {
    ...
    final bgColor = isUser ? AppColors.secondary 
                           : Colors.white70;
    final textColor = isUser ? Colors.white // Defina textUser
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            Container(
              decoration: BoxDecoration(
                color: AppColors.background, // Ou Theme.of(context).cardColor para contraste
                boxShadow: [
                  BoxShadow(
              ...
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10).copyWith(bottom: MediaQuery.of(context).padding.bottom /2 + 10), // Padding seguro inferior
              child: Row(
                children: [
                  Expanded(
                    child: ConstrainedBox(
                      constraints: const BoxConstraints(maxHeight: 120), // Limita altura do campo de texto
                      child: TextField(
